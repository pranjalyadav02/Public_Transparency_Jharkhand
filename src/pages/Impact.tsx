...
import { Map, Users, Target, Activity, Loader2 } from 'lucide-react';
...
export const Impact = () => {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/v1/public/impact/overview').then(res => res.json()),
      fetch('/api/v1/public/overview').then(res => res.json())
    ]).then(([impactRes, overviewRes]) => {
      if (impactRes.success) setImpactData(impactRes.data);
  ...
  if (!impactData || !overview) return null;
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6 border-t border-blue-800 pt-8">
            <div>
              ...
              <div className="text-3xl font-bold">{(overview.beneficiaries.value / 1000000).toFixed(1)}M</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-3xl font-bold">{overview.deployedSolutions.value.toLocaleString()}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-3xl font-bold">₹{(overview.csrFunding.value / 10000000).toFixed(0)} Cr</div>
            </div>
          </div>
        </div>
      </div>
              ...
              <CardContent>
                <div className="h-80 w-full mt-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={impactData.domainData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="font-semibold text-gray-900">{(impactData.expectedVsActual.expected / 1000).toFixed(0)}K</span>
                      </div>
                      <div className="w-full bg-gray-100 h-1.5 rounded-full"><div className="bg-gray-400 h-full w-[94%] rounded-full"></div></div>
                    </div>
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-gray-500 font-medium">Reported by Partners</span>
                        <span className="font-semibold text-gray-900">{(impactData.expectedVsActual.reported / 1000).toFixed(0)}K</span>
                      </div>
                      <div className="w-full bg-gray-100 h-1.5 rounded-full"><div className="bg-blue-400 h-full w-[100%] rounded-full"></div></div>
                    </div>
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-emerald-600 font-medium">Verified by Govt / Community</span>
                        <span className="font-semibold text-emerald-700">{(impactData.expectedVsActual.verified / 1000).toFixed(0)}K</span>
                      </div>
                      <div className="w-full bg-gray-100 h-1.5 rounded-full"><div className="bg-emerald-500 h-full w-[96%] rounded-full"></div></div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                      <div className="text-xl font-bold text-gray-900">{impactData.sla.firstResponse}</div>
                      <div className="text-xs text-gray-500 mt-1">First Response</div>
                    </div>
                    <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                      <div className="text-xl font-bold text-gray-900">{impactData.sla.verification}</div>
                      <div className="text-xs text-gray-500 mt-1">Verification</div>
                    </div>
                    <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                      <div className="text-xl font-bold text-gray-900">{impactData.sla.resolution}</div>
                      <div className="text-xs text-gray-500 mt-1">Avg Resolution</div>
                    </div>
                    <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-100">
                      <div className="text-xl font-bold text-emerald-700">{impactData.sla.compliance}%</div>
